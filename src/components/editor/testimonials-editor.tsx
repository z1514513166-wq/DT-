...
export default function TestimonialsEditor({
  ...
  const add = () => {
    onChange([
      ...testimonials,
      { id: crypto.randomUUID(), name: '', role: '', quote: '', avatarUrl: null },
    ]);
  };
  ...
  const update = (id: string, field: keyof Testimonial, value: string) => {
    onChange(
  ...
  return (
    <div className="space-y-4">
      ...
      <Input
        label="区块标题"
        value={testimonialsTitle ?? '客户评价'}
        onChange={(e) => onTitleChange(e.target.value)}
        placeholder="客户评价"
      />

      {testimonials.length === 0 && (
        <p className="text-gray-500 text-sm">暂无用户评价。</p>
      )}
      ...
      {testimonials.map((t) => (
        ...
        >
          ...
          <Textarea
            ...
            placeholder="这个产品真的很棒..."
            rows={3}
          />
          <Input
            label="头像链接（选填）"
            value={t.avatarUrl || ''}
            onChange={(e) => update(t.id, 'avatarUrl', e.target.value)}
